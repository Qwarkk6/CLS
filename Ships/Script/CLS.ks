...
set launchLoc to ship:geoposition.				// Records liftoff geo-location for downrange distance calc
...
Set printlist to List(). Set printlisthistory to List(). printlisthistory:add("*"). set currentline to 6.
...
Until launchcomplete {
	...
	if csvLog and missiontime > 0 {
		log_data(missiontime,LIST(missiontime,mode,StageDV(PayloadProtection),twr(),throttle,pitch_for(ship),(ship:q*constant:AtmToKPa),ship:altitude,ship:apoapsis,eta:apoapsis,ship:periapsis,currentstagenum,staginginprogress,runmode,numparts),logPath).
	}
	...
	If runmode = 2 {	
					...
					If CentralEnginesThrottle = true {
						For engine in Ship:partstaggedpattern("^CentralEngine") {
							If Time:seconds - throttletime < 5 {
								...
								lock throttle to ((1 - currentthrottle)*((Time:seconds-throttletime)/5)+currentthrottle).
							} else {
		...
		If currentstagenum > 1 and staginginprogress = false {
			...
			// Checks whether it has been 5 seconds since staging and time to apoapsis is above 135 seconds before gradually throttling to TWR set by UpperAscentTWR			
			If Eta:apoapsis > 135 and broke90 = false and (Time:seconds - stagefinishtime) >= 5 {
				Set broke90 to true.
				Set throttletime to Time:seconds.
				Set currentthrottle to throttle.
